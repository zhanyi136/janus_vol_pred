...
if __name__ == "__main__":
    ...
    features_input_dir = Path(research_cfg["output_root"]) / research_cfg["features_output_dir"]
    results_output_dir = Path(research_cfg["output_root"]) / research_cfg["results_output_dir"]
        ...
        for window_dates in tqdm(windows, desc=f"{symbol}"):
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    # 验证所有天的结果
                    # 多天一个窗口时，只要窗口里有一天没成功，实际上整个窗口都会重新训练，并且结果都会被覆盖。
                    all_ok = True
                    for d in window_dates:
                        out_dir = results_output_dir / d / symbol
                        if verify_train_result(out_dir):
